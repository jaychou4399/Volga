    ...
    <div className="w-full max-w-6xl mx-auto px-4 sm:px-10 py-10 relative z-10">
      <div className="mb-8 flex flex-col items-center md:items-start">
        ...
        <div className="text-center md:text-left w-full">
          <h1 className="text-4xl font-black text-slate-900 dark:text-white mb-2 tracking-widest drop-shadow-sm uppercase">
            项目矩阵
          </h1>
          <p className="text-slate-600 dark:text-slate-400">
            GitHub 开源仓库 · 共 {projects.length} 个项目
          </p>
        </div>
      </div>
      ...
      <div className="mb-12 flex justify-center w-full">
        <div className="relative w-full max-w-lg">
          <input
            ...
            className="w-full bg-white/40 dark:bg-slate-800/50 backdrop-blur-md border border-white/40 dark:border-white/10 rounded-full px-6 py-3 pl-12 text-slate-800 dark:text-white shadow-xl focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all placeholder-slate-500"
          />
      ...
      <motion.div layout className="grid grid-cols-1 md:grid-cols-2 gap-6 relative">
          ...
          {filteredProjects.map((project) => (
            ...
            >
              <a
                ...
                className="block h-full rounded-3xl bg-white/60 dark:bg-slate-800/50 backdrop-blur-xl border border-white/40 dark:border-white/10 shadow-xl overflow-hidden hover:shadow-indigo-500/20 transition-all duration-500 hover:-translate-y-1 group relative p-6 md:p-8"
              >
                ...
                <div className="flex items-start justify-between mb-3 relative z-10">
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    ...
                    <svg className="w-6 h-6 text-slate-400 group-hover:text-slate-800 dark:group-hover:text-white transition-colors" fill="currentColor" viewBox="0 0 24 24">
                ...
                <p className="text-sm text-slate-700 dark:text-slate-300 leading-relaxed line-clamp-3 mb-5 relative z-10 min-h-[40px]">
                  {project.description}
                </p>
        ...
        {filteredProjects.length === 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            className="col-span-full text-center py-20 text-slate-500 w-full">
            没有找到匹配的项目
          </motion.div>
        )}
      </motion.div>
